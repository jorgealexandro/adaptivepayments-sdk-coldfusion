	...
	public any function deserialize(any jsonObj)
	{
		...
		if(isStruct(local.json))
		{
			...
			for(key in local.json)
			{
					...
					if('#key#' eq currName)
						if(NOT isStruct(local.json[key]) AND ('InvoiceItem' eq "string" OR 'InvoiceItem' eq "boolean" OR 'InvoiceItem' eq "numeric"))
						{
							this.setitem(local.json[key]);
						} else {
							var keyCom = 'item';
							if(keyCom eq 'ack')
								keyCom = 'ackCode';
					...
					if('#key#' eq currName)
						if(NOT isStruct(local.json[key]) AND ('numeric' eq "string" OR 'numeric' eq "boolean" OR 'numeric' eq "numeric"))
						{
							...
							var keyObj = createObject("component",'#keyCom#');
							this.settotalTax( keyObj.deserialize(local.json[key]) );
					...
					if('#key#' eq currName)
						if(NOT isStruct(local.json[key]) AND ('numeric' eq "string" OR 'numeric' eq "boolean" OR 'numeric' eq "numeric"))
						{
							...
							var keyObj = createObject("component",'#keyCom#');
							this.settotalShipping( keyObj.deserialize(local.json[key]) );
			...
			for( i = 1; i lte ArrayLen(local.json); i = i + 1)
			{
				...
				for(key in local.json[i])
				{
					...
					if('#key#' eq currName)
							if(NOT isStruct(local.json[i][key]) AND ('InvoiceItem' eq "string" OR 'InvoiceItem' eq "boolean" OR 'InvoiceItem' eq "numeric"))
							{
								this.setitem(local.json[i][key]);
							} else {
								var keyCom = 'item';
								if(keyCom eq 'ack')
									keyCom = 'ackCode';
					...
					if('#key#' eq currName)
							if(NOT isStruct(local.json[i][key]) AND ('numeric' eq "string" OR 'numeric' eq "boolean" OR 'numeric' eq "numeric"))
							{
								...
								var keyObj = createObject("component",'#keyCom#');
								this.settotalTax( keyObj.deserialize(local.json[i][key]) );
					...
					if('#key#' eq currName)
							if(NOT isStruct(local.json[i][key]) AND ('numeric' eq "string" OR 'numeric' eq "boolean" OR 'numeric' eq "numeric"))
							{
								...
								var keyObj = createObject("component",'#keyCom#');
								this.settotalShipping( keyObj.deserialize(local.json[i][key]) );
